...
import { RefutationLogo } from '@/components/ui/RefutationLogo';

export function Header() {
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-slate-900/80 backdrop-blur-md border-b border-slate-800">
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2 text-xl font-bold text-brand-light hover:text-brand-accent transition">
          <RefutationLogo size={28} />
          <span className="hidden sm:inline">Refutation</span>
        </Link>
